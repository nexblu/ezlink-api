...
@auth_router.post("/illustra-line/v1/auth/refresh-token")
@jwt_required(refresh=True)
async def refresh_token():
    return await auth_controller.refresh_token()


@auth_router.get("/illustra-line/v1/auth/email-verify/<string:token>")
async def account_active(token):
    return await auth_controller.get_account_active(token)


@auth_router.post("/illustra-line/v1/auth/email-verify")
...
@auth_router.route(
    "/illustra-line/v1/auth/reset/reset-password/<string:token>",
    methods=["POST", "GET"],
)
...
@auth_router.post("/illustra-line/v1/auth/reset/email-reset-password")
...
@auth_router.post("/illustra-line/v1/auth/login")
async def login():
    data = request.json
    email = data.get("email")
...
@auth_router.post("/illustra-line/v1/auth/register")
async def register():
    data = request.json
